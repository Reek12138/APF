...
class APF_env:
    ...
    def reset(self):
        ...
        self.leader_agent.set_position(self.width * 0.10 + np.random.rand() * self.width * 0.8,
                                       self.height * 0.10 + np.random.rand() * self.height * 0.8)
        # self.leader_agent.set_position(np.random.rand() * 10 + 5, np.random.rand() * 10 + 5)

        for i in range(self.follower_uav_num):
            self.follower_uavs[f"follower_{i}"].set_position(self.leader_agent.pos[0] + self.formation_pos[i][0],
                                                             self.leader_agent.pos[1] + self.formation_pos[i][1])
        ...
        while np.linalg.norm(np.array(self.leader_target_pos) - np.array(
                self.leader_agent.pos)) < self.agent_radius + self.target_radius + self.safe_theta * 3 or flag1:
            self.leader_agent.set_position(self.width * 0.10 + np.random.rand() * self.width * 0.8,
                                           self.height * 0.10 + np.random.rand() * self.height * 0.8)
            # self.leader_agent.set_position(np.random.rand() * 10 + 5, np.random.rand() * 10 + 5)
    ...
    def caculate_force(self, target_pos, self_pos, obs_num, is_follower=False, self_id=0):
        ...
        if not is_follower:
            ...
            k_rep = 30 if target_dis > 10 else 8
            k_goal = 1
            k_att = 1 if target_dis >= 15 else 10 # 保证接近目标点时的速度

            force_attractive = [target_dis * k_att, target_angle] # 领导者引力
            force_list.append(force_attractive)

            for obs_id, obs in self.obstacles.items(): # 障碍斥力（改进）
                dis_, angle_ = APF_env.calculate_relative_distance_and_angle(self_pos,[obs.pos_x, obs.pos_y])
                angle = (angle_ + np.pi) % (2 * np.pi) # 反向
                dis = dis_ - self.obs_radius - self.agent_radius # 边缘间距

                # === 改进斥力 ===，
                f_rep1 = k_rep * (1 / dis) * (target_dis ** k_goal) * (1 / dis) ** 2  # 方向为angle
                f_rep2 = k_goal / 2 * k_rep * (1 / dis ) ** 2 * target_dis ** (k_goal - 1) # 方向为target_angle

                #斥力范围内添加斥力
                if dis <= self.obs_delta * 2 - self.obs_radius - self.agent_radius: #斥力范围 20-2-1
                    force_list.append([f_rep1, angle])
                    force_list.append([f_rep2,target_angle])

        # === 跟随者 ===
        else:
            k_att = 1 if target_dis <= 1 else 10  # 保证接近目标点时的速度
            k_rep = 20 if target_dis > 10 else 5
            if self.leader_agent.done and target_dis < 0.5: # 接近目标时减少障碍斥力防止到不了
                k_rep = 1
            k_goal = 1

            # 目标点引力
            force_attractive = [target_dis * k_att, target_angle]
            force_list.append(force_attractive)  # 领导者引力
            ...
            # # 领导者斥力
            # dis_, angle_ = APF_env.calculate_relative_distance_and_angle(self_pos, self.leader_agent.pos)
            # angle = (angle_ + np.pi) % (2 * np.pi)  # 反向
            # dis = dis_ - 2 * self.agent_radius  # 边缘间距
            #
            # # 改进斥力
            # f_rep1 = k_rep * (1 / dis) * (target_dis ** k_goal) * (1 / dis) ** 2  # 方向为angle
            # f_rep2 = k_goal / 2 * k_rep * (1 / dis) ** 2 * target_dis ** (k_goal - 1)  # 方向为target_angle
            #
            # if dis <= self.agent_radius*2:
            #     force_list.append([f_rep1, angle])
            #     force_list.append([f_rep2, target_angle])
            ...
            for j in range(self.follower_uav_num):
                if j != self_id:
                    dis_, angle_ = APF_env.calculate_relative_distance_and_angle(self_pos, self.follower_uavs[
                        f"follower_{j}"].position())
    ...
    def is_Collision(self):
        ...
        for obs_id, obs in self.obstacles.items():
            leader_dis, _ = self.calculate_relative_distance_and_angle(self.leader_agent.pos, [obs.pos_x, obs.pos_y])
            if leader_dis <= self.agent_radius + self.obs_radius:
                return True

            for i in range(self.follower_uav_num):
                follower_dis, _ = self.calculate_relative_distance_and_angle(self.follower_uavs[f"follower_{i}"].pos,
                                                                             [obs.pos_x, obs.pos_y])
                if follower_dis <= self.agent_radius + self.obs_radius:
                    return True
        # 无人机间距离
        follower_pos = []
        for i in range(self.follower_uav_num):
            follower_pos.append(self.follower_uavs[f"follower_{i}"].pos)
        distance_matrix = cdist(follower_pos, follower_pos)
        np.fill_diagonal(distance_matrix, np.inf)
        if np.any(distance_matrix < 2 * self.agent_radius):
            return True
        return False
    ...
    def isTargetReached(self):
        ...
        返回：
        all_done(bool): 任务完成标志
        """
        all_done = True
        if np.linalg.norm(np.array(self.leader_agent.pos)-np.array(self.leader_target_pos))<=self.target_radius/2:
            self.leader_agent.done=True # 标志leader到目标点
        for i in range(self.follower_uav_num):
            if np.linalg.norm(np.array(self.follower_uavs[f"follower_{i}"].pos)-np.array(self.leader_agent.pos)
                              -np.array(self.formation_pos[i])) <= 0.4:
                self.follower_uavs[f"follower_{i}"].target=True
                if self.leader_agent.done:
...
for episode_i in range(EPISODE_NUM):
    ...
    for step_i in range(STEP_NUM):
        ...
        steer_error = (force_angle - current_orientation) % (2 * np.pi) # 角度差范围[-2pi,+2pi]->[0,2pi]
        if steer_error > math.radians(180): # 大于pi的部分变负
            steer_error = steer_error - 2 * np.pi


        if random.random() < 0.1: # 加随机扰动防止陷入局部最优
            steer_error += random.uniform(-0.05, 0.05)
        ...
        for i in range(env.follower_uav_num):
            force_ = env.caculate_force(target_pos=np.add(env.leader_agent.pos,env.formation_pos[i]).tolist(),
                                        self_pos=env.follower_uavs[f"follower_{i}"].pos,
                                        obs_num=15,
                                        is_follower=True,
                                        self_id=i)
            vx_ = force_[0] * cos(force_[1])
            vy_ = force_[0] * sin(force_[1])
            if random.random() < 0.1:
                vx_ += random.uniform(-0.1, 0.1)
                vy_ += random.uniform(-0.1, 0.1)

            vx.append(np.clip(force_[0] * cos(force_[1]), -1.2, 1.2))
            vy.append(np.clip(force_[0] * sin(force_[1]), -1.2, 1.2))

        # === 执行动作 ===
        if not env.leader_agent.done:
            print(f"leader:线速度{linear_vel}, 角速度{steer_vel}")
            env.leader_agent.take_action(linear_vel, steer_vel, dt=0.1)
        else:
            force=[0,0]
        for i in range(env.follower_uav_num):
            if not env.follower_uavs[f"follower_{i}"].done:
                print(f"follower:vx:{vx[i]},vy:{vy[i]}")
                env.follower_uavs[f"follower_{i}"].take_action(vx[i],vy[i])
    ...
    env.render_close()
    print(f"成功率: {success_num / EPISODE_NUM}")
    print(f"成功时平均步数: {total_steps / success_num}")
    print(f"碰撞次数: {collision_num}")
